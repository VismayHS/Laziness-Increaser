...
const TextType = ({
  ...
  const normalizedVariableSpeed = useMemo(() => {
  ...
  const sourceText = useMemo(() => {
    ...
    if (typeof texts !== "undefined") return texts;
    return "";
  }, [text, texts]);

  const textArray = useMemo(() => {
    const arr = Array.isArray(sourceText) ? sourceText : [sourceText];
    const cleaned = arr.filter((entry) => typeof entry === "string");
    return cleaned.length > 0 ? cleaned : [""];
  }, [sourceText]);

  const getRandomSpeed = useCallback(() => {
    if (!normalizedVariableSpeed) return typingSpeed;
    const { min, max } = normalizedVariableSpeed;
    return Math.random() * (max - min) + min;
  }, [normalizedVariableSpeed, typingSpeed]);
    ...
    const executeTypingAnimation = () => {
      if (isDeleting) {
        ...
        timeout = setTimeout(
          ...
          normalizedVariableSpeed ? getRandomSpeed() : typingSpeed
        );
